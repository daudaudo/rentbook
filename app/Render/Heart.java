...

                {
                    ...
                    addCardBtn.setOnAction((ActionEvent event) -> {
                        if(req.session().get("user")==null){
                            ...
                            ArrayList<HashMap<String, Object>> check =  DB.belong().prepare("SELECT * FROM cart where username = ? and book_id = ?").binding(req.session().get("user"),getTableView().getItems().get(getIndex()).book_id).get();
